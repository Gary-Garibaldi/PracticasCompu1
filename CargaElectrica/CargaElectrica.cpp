...
int main()
{
    ...
    cout << "Carga de la molecula = " << cargaMolecula << endl;
...
int CalcularCargaApH(float& ph, grupos todosGrupos)
{
    ...
    //si el pka es mayor al ph el grupo se protona, si el pka es menor al ph el grupo se desprotona

    for (int i = 0; i < 3; i++){

        if(todosGrupos[i].getpka() > ph){
            todosGrupos[i].setCarga(1);
        }

        cargaTotal += todosGrupos[i].getCarga();
    }
